<template>
  <div>
    <h2 clas="tituloAldeia"><b>{{this.id}}</b></h2>
    <h3>Casas</h3>
    ...
    <button class="enviar" v-if="formularioShow"
      ...
      <div id="formularioId" v-if="!formularioShow">
        <form id="form" v-on:submit.prevent="addUser" v-if="!enviado">
          <div>
            ...
            <input id="cidade" v-model="newUsers.cidade" />
          </div>
          <div>
            <p>Aldeia de residencia</p>
            <select class="opcoes" name="aldeias" v-model="newUsers.aldeia">
...
export default {
  ...
  firestore() {
    return {
      ...
      casas: db.collection("morador").doc(this.id).collection("casa").orderBy("numero", "asc"),
    };
  },
  methods: {
    addUser: function () {
      db.collection("morador")
        .doc(this.newUsers.aldeia)
        .collection("casa")
        .add({
